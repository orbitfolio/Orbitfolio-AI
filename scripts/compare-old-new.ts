 ...
 */
import { fetchChart, fetchQuoteSummary } from '../lib/market/yahoo';
import { computeTechnicals, rsi, macd, sma, type OhlcvBar } from '../lib/market/technicals';
import { computeFundamentals } from '../lib/market/fundamentals';
import { scoreAnalystConsensus } from '../lib/market/analyst';
import { combineRating, actionFromScore, labelFromScore } from '../lib/market/rating';

function clamp(n: number, min = 0, max = 10) {
  return Math.min(max, Math.max(min, n));
}

function oldLabel(score: number) {
  if (score >= 7.5) return 'Robust';
  if (score >= 6.0) return 'Constructive';
  if (score >= 4.5) return 'Mixed';
  if (score >= 3.0) return 'Cautious';
  return 'Fragile';
}

function oldCombine(tech: number, fund: number, analyst: number | null) {
  const hasA = analyst != null && Number.isFinite(analyst);
  let wF = 0.4, wT = 0.35, wA = hasA ? 0.25 : 0;
  const s = wF + wT + wA;
  wF /= s; wT /= s; wA /= s;
  const raw = fund * wF + tech * wT + (hasA ? (analyst as number) * wA : 0);
  const orbitScore = Math.round(clamp(raw) * 10) / 10;
  return { orbitScore, label: oldLabel(orbitScore), wF, wT, wA, tech: Math.round(tech*10)/10, fund: Math.round(fund*10)/10, analyst: hasA ? Math.round((analyst as number)*10)/10 : null };
}

function oldScoreTechnicals(bars: OhlcvBar[]) {
  ...
  if (lastClose != null && sma50 != null && sma200 != null) {
    let trend = 5;
    if (lastClose > sma50) trend += 1.5; else trend -= 1.5;
    if (lastClose > sma200) trend += 1.5; else trend -= 1.5;
    if (sma50 > sma200) trend += 1; else trend -= 1;
...
function scoreMargins(m: number) {
...
function oldScoreFundamentals(summary: any, bars: OhlcvBar[]) {
  const parts: number[] = [];
  const used: string[] = [];
  ...
  let week52: number | null = null;
  const last = summary.lastPrice ?? bars.at(-1)?.close;
  const hi = summary.fiftyTwoWeekHigh ?? Math.max(...bars.slice(-252).map((b: OhlcvBar) => b.high));
  const lo = summary.fiftyTwoWeekLow ?? Math.min(...bars.slice(-252).map((b: OhlcvBar) => b.low));
  if (Number.isFinite(last) && Number.isFinite(hi) && Number.isFinite(lo) && hi > lo) {
    week52 = (last - lo) / (hi - lo);
  }
  if (Number.isFinite(pe)) { parts.push(scorePE(pe)); used.push('trailingPE'); }
  if (Number.isFinite(pb)) { parts.push(scorePB(pb)); used.push('priceToBook'); }
  if (Number.isFinite(roe)) { parts.push(scoreROE(roe)); used.push('returnOnEquity'); }
  if (Number.isFinite(pm)) { parts.push(scoreMargins(pm)); used.push('profitMargins'); }
  if (Number.isFinite(de)) { parts.push(scoreDE(de)); used.push('debtToEquity'); }
  if (week52 != null) { parts.push(score52w(week52)); used.push('week52Position'); }
  const score = parts.length ? Math.round((parts.reduce((a, b) => a + b, 0) / parts.length) * 10) / 10 : 5;
  return { score, used, week52, pe, pb, roe };
}

function oldAnalyst(summary: any) {
  const snap = scoreAnalystConsensus(summary.recommendationTrend, {
  ...
  if (!counts) return { score: null as number | null, available: false };
  const total = counts.strongBuy + counts.buy + counts.hold + counts.sell + counts.strongSell;
  if (total <= 0) return { score: null as number | null, available: false };
  const weighted =
...
async function one(symbol: string, market: string) {
  const chart = await fetchChart(symbol, '1y', '1d');
  ...
  const newT = computeTechnicals(bars);
  const newF = computeFundamentals({
  ...
  const newA = scoreAnalystConsensus(summary?.recommendationTrend, {
    targetMean: summary?.targetMean,
    recommendationKey: summary?.recommendationKey,
    numberOfAnalystOpinions: summary?.numberOfAnalystOpinions,
...
async function main() {
  const rows = [];
  for (const [sym, mkt] of [['AAPL', 'USA'], ['SHOP.TO', 'Canada'], ['RELIANCE.NS', 'India']] as const) {
    try {
